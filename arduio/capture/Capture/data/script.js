...
let isStoppingPreview = false; // suppress error UI when stop is intentional
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 初期状態でcaptureボタンを無効化
    disableCaptureButton('ライブプレビューを開始してください');
...
function stopPreview() {
    ...
    if (streamImg) {
        try {
            // この後に発火する onerror/onload を抑止（ユーザー操作による停止をエラー扱いしない）
    ...
    if (previewArea) {
        previewArea.innerHTML = '<div class="preview-placeholder">⏹ ライブプレビューを停止しました<br>📹 「プレビュー開始」で再開できます<br>📁 左の一覧から写真を選んで表示することもできます</div>';
    }
    if (previewBtn) {
        previewBtn.textContent = '📹 プレビュー開始';
        previewBtn.title = 'ライブプレビューを開始します';
        previewBtn.classList.remove('is-live');
    }

    // ストリーミング停止時はcaptureボタンを無効化
    disableCaptureButton('プレビュー停止中');
    // ユーザへのフィードバック
    try { showMessage('⏹ ライブプレビューを停止しました\n📹 「プレビュー開始」で再開できます', 'success'); } catch (e) {}
    isLivePreview = false;

    // 少し後にフラグを戻す（onerrorが遅れて来る環境を考慮）
    setTimeout(() => { isStoppingPreview = false; }, 500);
}
...
function startPreview() {
    ...
    // Loading状態を表示
    previewArea.innerHTML = '<div class="preview-placeholder">📡 ストリームに接続中...</div>';

    // 5秒後にタイムアウトチェック
    const timeoutId = setTimeout(() => {
        if (isLivePreview && (!img.naturalWidth || img.naturalWidth === 0)) {
            console.warn('⚠️ Stream timeout - no image data received');
            previewArea.innerHTML = '<div class="preview-placeholder">⏱️ タイムアウト<br>ESP32-CAM がビジーまたはオフラインの可能性があります</div>';
            previewBtn.textContent = '📹 プレビュー開始';
            previewBtn.title = 'ライブプレビューを開始します';
            previewBtn.classList.remove('is-live');
            isLivePreview = false;
    ...
    img.onerror = function() {
        ...
        if (isStoppingPreview) {
            console.log('🛑 Preview was stopped by user. Suppressing connection error UI.');
            isStoppingPreview = false; // 念のため戻す
            return; // ユーザー主導の停止なのでエラーUIは出さない
        }

        console.error('❌ プレビュー接続に失敗しました');
        previewArea.innerHTML = '<div class="preview-placeholder">❌ プレビュー接続に失敗しました<br>ESP32-CAM の電源・Wi‑Fi状態を確認し、再度「プレビュー開始」を押してください</div>';
    previewBtn.textContent = '📹 プレビュー開始';
    previewBtn.title = 'ライブプレビューを開始します';
        try { previewBtn.classList.remove('is-live'); } catch (e) {}
        isLivePreview = false;

        // ストリーミング失敗時はcaptureボタンを無効化（日本語メッセージ）
        disableCaptureButton('プレビュー接続に失敗');
        return;
    };

    previewBtn.textContent = '⏹ プレビュー停止';
    previewBtn.title = 'ライブプレビューを停止します';
    previewBtn.classList.add('is-live');
    isLivePreview = true;
}
...
function previewFile(filename) {
    ...
    previewArea.innerHTML = '<div class="preview-placeholder">📷 画像を読み込み中...<br><div class="loading-spinner"></div></div>';
    ...
    const progressTimer = setInterval(() => {
        if (!img.complete && !progressReported) {
            ...
            previewArea.innerHTML = '<div class="preview-placeholder">📷 画像を読み込み中... (5秒)<br><div class="loading-spinner"></div></div>';
    ...
    const loadingTimeout = setTimeout(() => {
        ...
        if (img && !img.complete) {
        console.error('⏰ 画像の読み込みが10秒でタイムアウト:', filename);
            ...
            previewArea.innerHTML = '<div class="preview-placeholder">⏱️ 読み込みタイムアウト（10秒）<br>画像が大きいかサーバがビジーの可能性があります<br><button onclick="previewFile(\'' + filename + '\')">🔄 再試行</button></div>';
        }
    }, 10000);
    ...
    img.onerror = function(event) {
        clearTimeout(loadingTimeout);
        clearInterval(progressTimer);
    console.error('❌ 画像の読み込みエラー:', filename);
    ...
    previewArea.innerHTML = '<div class="preview-placeholder">❌ 画像の読み込みに失敗しました<br>' + filename + '<br><button onclick="previewFile(\'' + filename + '\')">🔄 再試行</button></div>';
    };
    ...
    fetch('/app/files', { method: 'HEAD' })
        .then(() => console.log('🌐 ネットワーク接続: OK'))
        .catch(() => console.warn('🌐 ネットワーク接続: 問題の可能性'));

    // Additional timeout check for incomplete loading
    setTimeout(() => {
        if (img && img.src && !img.complete && img.naturalWidth === 0) {
            console.warn('⏳ 5秒経過しても画像を読み込み中:', filename);
            ...
            if (previewArea.innerHTML.includes('画像を読み込み中...')) {
                previewArea.innerHTML = previewArea.innerHTML.replace('</div>', debugInfo + '</div>');
            }
            previewArea.innerHTML = '<div class="preview-placeholder">📷 まだ読み込み中...<br>大きな画像の可能性があります<br><button onclick="showPreview(\'' + filename + '\')">キャンセル</button></div>';
        }
    }, 5000);
}
        ...
        .then(data => {
            if (data.success) {
                ...
                if (previewArea && previewArea.querySelector('img')?.alt === filename) {
                    previewArea.innerHTML = '<div class="preview-placeholder">📸 左の一覧から写真を選択してください</div>';
                }
            } else {
