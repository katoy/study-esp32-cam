...
void handleCaptureAPI() {
  ...
  if (!fb) {
  server.send(500, "application/json", "{\"success\":false,\"error\":\"カメラの撮影に失敗しました\"}");
    return;
  }
  ...
  if (!file) {
    esp_camera_fb_return(fb);
  server.send(500, "application/json", "{\"success\":false,\"error\":\"ファイル書き込みに失敗しました\"}");
    return;
  }
...
void handlePhoto() {
  if (!server.hasArg("name")) {
    server.send(400, "application/json", "{\"success\":false,\"error\":\"name パラメータがありません\"}");
    return;
  }
  ...
  if (!file) {
    server.send(404, "application/json", "{\"success\":false,\"error\":\"ファイルが見つかりません\"}");
    return;
  }
...
void handleDelete() {
  ...
  if (!SD_MMC.exists(name)) {
    server.send(404, "application/json", "{\"success\":false,\"error\":\"パスが見つかりません\"}");
    return;
  }
  ...
  if (ok) {
    ...
    server.send(500, "application/json", String("{\"success\":false,\"error\":\"削除に失敗しました: ") + name + "\"}");
  }
}
